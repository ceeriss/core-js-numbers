...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const firstStep = y2 - y1;
  const secondStep = x2 - x1;
  const result = Math.sqrt(secondStep * secondStep + firstStep * firstStep);
  return result;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const angel = Math.acos(proiz / (first * second));
  return angel;
}
getAngleBetweenVectors(1, 1, 1, 1);
/**
...
function getParallelepipedDiagonal(a, b, c) {
  ...
  const bSide = b * b;
  const cSide = c * c;
  const result = Math.sqrt(aSide + bSide + cSide);
  return result;
}
